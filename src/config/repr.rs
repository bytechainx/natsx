...
struct NatsConfigRepr {
    ...
    /// 慢消费者判定超时（毫秒）；缺省 `None` 表示回退 `operation_timeout`。
    slow_consumer_timeout_ms: Option<u64>,
...
impl Default for NatsConfigRepr {
    fn default() -> Self {
        ...
        Self {
            ...
            slow_consumer_timeout_ms: base.slow_consumer_timeout.map(duration_to_millis),
...
impl NatsConfigRepr {
    fn into_config(self) -> NatsResult<NatsConfig> {
        ...
        Ok(NatsConfig {
            ...
            slow_consumer_timeout: self.slow_consumer_timeout_ms.map(Duration::from_millis),
            name: self.name,
            tls: self.tls,
